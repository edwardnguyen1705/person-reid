...
class WarmupCosineAnnealingLR(lr_scheduler._LRScheduler):
    r"""Warmup learning rate,
    copied from here: https://github.com/JDAI-CV/fast-reid/blob/master/fastreid/solver/lr_scheduler.py
    """
    ...
    def get_lr(self):
        if self.last_epoch <= self.warmup_iters:
            warmup_factor = self.__get_warmup_factor_at_iter(
                self.warmup_method,
                self.last_epoch,
                self.warmup_iters,
                self.warmup_factor,
    ...
    def __get_warmup_factor_at_iter(
